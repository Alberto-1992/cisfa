...
if(isset($_GET['guarda'])){
    ...
    $idcont = $_GET['idcont'];
    $idcont2 = base64_encode($_GET['idcont']);
    $idcontar = $_GET['idcontar'];
    ...
    ?>
                    ...
                    <p style="text-align: center; color: red; font-size: 16px;">Para cancelar una orden de suministro, primero elimine cada una de las claves mostradas en la orden.</p>
                        ...
                        <input type="hidden" id="claveid" value="<?php echo $idcont; ?>" requerided="requireded" style="width: 200px; border: none; height: 35px; color: black; border-radius: 10px;" >
                        <input type="hidden" id="clavecontar" value="<?php echo $idcontar; ?>" requerided="requireded" style="width: 200px; border: none; height: 35px; color: black; border-radius: 10px;" >
...
<td class="col-lg-1" style=""> 
        <a href="eliminaRegistr?id_ordenSuministro='.$row['id_ordenSuministro'].'&claveMedicamento='.$row['claveHraei'].'&claveUnica='.$ID_usuario2.'&id_proveedor='.$idcont2.'&contrato='.$idcontar.'&cantidad='.$row['cantidad'].'" class="btn-danger" style="width: 80px; height: 25px; margin-top: 1%; border-radius: 4px; outline:none;" >Eliminar</button>
...
$var += $row['importe'];
     ...
     $('.elimina').click(function() {
         ...
         ob = {id_ordenSuministro:id_ordenSuministro};
     $.ajax({
                type: "POST",
     ...
     <td></td>
                        ...
                        <?php
                        /*
                         echo '<input type="submit "><a href="Orden.php?var='.$id_unico.'&valor2='.$claveUnica.'&total='.$var2.'&id_unico='.$id_unico.'&claveContrato='.$claveUnica.'" class="btn btn-info" style=" width: 150px; float: right; margin-right: 500px; margin-top: 0px;">GENERAR ORDEN</a>';
                         echo '<input type="submit "><a onclick="actualiza();" class="btn btn-danger" style=" width: 150px; float: right; margin-right: -330px; margin-top: 0px; color: white;">TOTAL</a>';
                      */
                        ...
                        $ID_usuario2 = base64_encode($ID_usuario);
                            echo '<a href="cancelarOrden?valor2='.$ID_usuario2.'" class="btn btn-danger" style=" width: 450px; float: left; margin-left: 40%; margin-top: 0px;">CANCELAR</a>';
                        }else{
