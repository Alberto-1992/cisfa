...
error_reporting(0);
 function formatMoney($number, $cents = 1) { // cents: 0=never, 1=if needed, 2=always
    ...
    $query = "SELECT * FROM listamedicamento where fechaContrato = '2023' and farmacia = 'gratuita' group by listamedicamento.id_medicamento desc ";
...
<table id="datatables-example"  class="table table-striped table-bordered nowrap table-darkgray table-hover"   width="100%"  >
    ...
    <?php
        while($dataRegistro= mysqli_fetch_assoc($row))
        {
			...
			<td><button type="button" id="elimina" value='<?php echo $clave ?>'  style="background: none; border: 0; color:inherit"> <i id ="eliminar" class="fas fa-trash"></i></button></td>
