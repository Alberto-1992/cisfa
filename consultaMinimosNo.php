...
$sql = $conexion2->query("SELECT COUNT(*) total FROM listamedicamento where cantidad < minimoconsumo and fechaContrato = '2022'");
...
cantidad, 
MINIMOPRECIO, 
MAXIMOPRECIO, LEFT(DESCRIPCION,400) AS DESCRIPCION, count(*) total, numeroContrato, farmacia FROM listamedicamento where cantidad < minimoconsumo and fechaContrato = '2022' group by listamedicamento.CLAVEHRAEI asc ";
...
if(isset($_POST['alumnos']))
{
    ...
    MAXIMOPRECIO, LEFT(DESCRIPCION,400) AS DESCRIPCION FROM listamedicamento where
		...
		listamedicamento.MAXIMOPRECIO  LIKE '%".$q."%' and fechaContrato = '2022' group by listamedicamento.CLAVEHRAEI";
}
...
if(!empty($buscarAlumnos) AND mysqli_num_rows($buscarAlumnos)>0)
{
    ...
    '
    <strong style="float: left; margin-left: 45%; font-size: 25px; font-style: italic; margin-top: -35px;"><label>Minimos no alcanzados <input type="text" value='.$fila['total'].'></label></strong>

        <table class="table table-striped table-darkgray table-hover" style="margin-top:40px; margin-left:10px;">
        
            <tr style="background-color: #7C7C7C; color: white; font-size:13px; font-style: italic">
            <th scope="col">Proveedor</th>
            <th scope="col">N° de contrato</th>
            <th scope="col">Famracia</th>
